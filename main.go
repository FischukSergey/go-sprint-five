...
// meanSpeed возвращает среднюю скорость бега или ходьбы. (km/h)
...
func (r Running) Calories() float64 {
	Kcal := (CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours
	return Kcal
}
...
func (w Walking) Calories() float64 {
	Kcal := (CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed()*KmHInMsec, 2)/(w.Height/CmInM)) * CaloriesSpeedHeightMultiplier * w.Weight) * w.Duration.Hours() * MinInHours
	return Kcal
}
...
func (s Swimming) Calories() float64 {
	Kcal := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return Kcal
}
